define([ 'jquery', 'underscore', 'backbone','helper' ], function($, _, Backbone) {
	...
	var wave = {
		...
		_draw: function($el, isManual){
			...
			var $parent = $el.parents("[sound_id]"),
				soundId = $parent.attr("sound_id"),
				waveUrl = $el.attr("sound_wave"),
				uploadId = $el.attr("sound_uploadId");
			if(!waveUrl || !uploadId) return;
		...
		draw : function(uploadId, wave){
			renderTaskManager.append([uploadId, wave]);
		},
		drawItem: function(uploadId, wave){
			var _this = this;
			_.each(this.$parents, function($parent, index){ 
				if($parent && $parent.find("[sound_uploadId="+uploadId+"]").size()){
					...
					if($el){
						...
						if($el.find(".player_playbar,player_seekbar,player_backgroundbar").size()){
                            ...
                            var waves = [];
							if($bgBar.size()){
                                waves.push({color:"#CCCCCC", container:$bgBar});
							}
							if($seekBar.size()){
                                waves.push({color:"#BBBBBB", container:$seekBar});
							}
							if($playBar.size()){
                                waves.push({color:"#FF6600", container:$playBar});
							}
							...
							$el.html(_this.drawWave({
                                ...
                                height:height,
                                isMask:true,
                                waves:[{color:"#FFFFFF", container:$el}]
                            }));
						}
	...
	var renderTaskManager =  new helper.TaskManager(wave.drawItem, function(){}, wave);
	window.wave = wave;
	return wave;
});