...
define(["jquery", "./face.lib"], function ($, facelib) {
    //jquery plugin
    // 获取文本框 光标位置
    jQuery.fn.extend({
        getCurPos: function () {
    ...
    Face.prototype.onEmotionClick = function () {
        ...
        this.el.on("click", function (event) {
            ...
            if (!title) return;
            title = "[" + title + "]";
            _this.textarea.triggerHandler("focus");
    ...
    face.init = function (srouce, textarea, option) {
        option = option || {};
        if (!instance) instance = new Face(srouce);
        var method = option.method || "click";
        $(srouce).unbind(method, addEvent)
            .bind(method, { textarea: $(textarea), width: option.width || 330 }, addEvent);
    };
